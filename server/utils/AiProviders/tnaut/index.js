...
class TNautLLM {
  constructor(embedder = null, modelPreference = null) {
    ...
    if (!process.env.TNAUT_ENDPOINT)
      throw new Error("No Azure API endpoint was set.");
    if (!process.env.TNAUT_KEY) throw new Error("No Azure API key was set.");

    this.apiVersion = "2024-12-01-preview";
    this.openai = new OpenAIClient(
      process.env.TNAUT_ENDPOINT,
      new AzureKeyCredential(process.env.TNAUT_KEY),
      {
        apiVersion: this.apiVersion,
      }
    );
    this.model = modelPreference ?? process.env.OPEN_MODEL_PREF;
    this.isOTypeModel = process.env.TNAUT_MODEL_TYPE === "reasoning" || false;
    this.limits = {
  ...
  #log(text, ...args) {
    console.log(`\x1b[32m[TNaut]\x1b[0m ${text}`, ...args);
  }
  ...
  streamingEnabled() {
    ...
    if (this.isOTypeModel) {
      this.#log(
        "Streaming will be disabled. TNAUT_MODEL_TYPE is set to 'reasoning'."
      );
      return false;
    }
  ...
  static promptWindowLimit(_modelName) {
    return !!process.env.TNAUT_TOKEN_LIMIT
      ? Number(process.env.TNAUT_TOKEN_LIMIT)
      : 4096;
  }
  ...
  /**
   * Handles the stream response from the TNaut API.
   ...
   * @param {import('../../helpers/chat/LLMPerformanceMonitor').MonitoredStream} stream - the stream response from the TNaut API w/tracking
